...
<body>
          ...
          <div class="modal-content">
            ...
            <form class="needs-validation" novalidate="" action="#" method="POST" id="form-aksi">
              ...
              <div class="modal-body">
                ...
                <div class="form-group mb-1">
                  <label>Lokasi Detail Kantor</label>
                  <select name="lokasi_kantor" id="lokasi_kantor" class="form-control select2" required="">
                    <option value=""> --- Pilih Kantor ---</option>
                    <?php
                    foreach ($master_kantor as $lok) {
                      echo "<option value=\"" . $lok->kode_ktr . "\">" . $lok->nama_kantor . "</option>";
                    }
                    ?>
                  </select>
                  <div class="invalid-feedback">
                    Lokasi Detail Kantor harus dipilih
                  </div>
                </div>
              </div>
  ...
  <script type="text/javascript">
    ...
    table = $("#table-1").DataTable({
      ...
      "order": [],
      "ajax": {
        "url": "<?php echo base_url() ?>master/get-data-lokasi",
        "type": "POST"
      },
      "ordering": false,
    ...
    function ubah(id) {
      ...
      $.ajax({
        url: "<?= base_url() ?>master/get-lokasi/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data) {
          $('input[name="id_lokasi"]').val(data.idlokasi);
          $('input[name="nama_lokasi"]').val(data.nama_lokasi);
          $('select[name="lokasi_kantor"]').val(data.kantor);
          $('#modal-aksi').modal('show');
        },
    ...
    $('#form-aksi').submit(function(event) {
      ...
      if ($('#form-aksi')[0].checkValidity() === false) {
        ...
        //proses post data ke database
        $.ajax({
          url: "<?= base_url() ?>master/lokasi-aksi",
          type: "POST",
          data: {
            id_lokasi: $('input[name="id_lokasi"]').val(),
            nama_lokasi: $('input[name="nama_lokasi"]').val(),
            lokasi_kantor: $('select[name="lokasi_kantor"]').val(),
            action_method: action_method,
          },
          dataType: "JSON",
          //jika data sukses tersimpan
          success: function(data) {
            if (data.status) {
